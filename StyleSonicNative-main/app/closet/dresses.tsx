      ...
      try {
        ...
        if (userId) {
          ...
          if (userDoc.exists()) {
            ...
            const dressesList = userData.dresses || [];
            // Ensure each dress item has an imageUrl
            const dressesWithUrls = dressesList.map((dress: any) => ({
              ...dress,
              imageUrl:
                dress.imageUrl ||
                `http://192.168.0.16:5000/uploads/${dress.imageId}`,
            }));
            setDresses(dressesWithUrls);
          }
        }
      } catch (error) {
        console.error("Error fetching dresses:", error);
        Alert.alert("Error", "Failed to fetch dresses");
      }
    };
